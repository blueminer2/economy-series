...
class ChestShop implements Plugin
{
	...
	public function eventHandler(&$data, $event)
	{
		switch($event)
		{
			case "tile.update":
				...
				console("salenum clear");
				$price = $data->data['Text3'];
				if(!is_numeric($price) or $price < 0) break;
				console("price clear");
				$productID = $this->isItem($data->data['Text4']);
				if($productID === false) break;
				console("productid clear");
				$chest = $this->adjacentCheck($data->x, $data->y, $data->z);
				if($chest === false) break;
				console("chest clear");
				if(strlen($shopOwner) > 15)
				{
					$data->data['Text1'] = substr($shopOwner, 0, 15);
				}
				else
				{
	 		...
	 		case "player.block.touch":
				$tiles = $this->api->tileentity->get($data['target']->x, $data['target']->y, $data['target']->z);
				if($tiles === false) break;
				$class = $tiles[0]->class;
				...
				switch($class)
				{
					case TILE_SIGN:
						switch($data['type'])
						{
							case "place":
								...
								foreach ($cfg as $val)
								{
									if($data['target']->x === $val['signX'] and $data['target']->y === $val['signY'] and $data['target']->z === $val['signZ'])
									{
										$c = $this->adjacentCheck($data['target']->x, $data['target']->y, $data['target']->z);
										if($c === false) break;
							...
							case "break":
								foreach ($cfg as $val)
								{
									if($val['signX'] === $data['target']->x and $val['signY'] === $data['target']->y and $val['signZ'] === $data['target']->z)
									{
					...
					case TILE_CHEST:
						foreach ($cfg as $val)
						{
							if($val['chestX'] === $data['target']->x and $val['chestY'] === $data['target']->y and $val['chestZ'] === $data['target']->z and $val['shopOwner'] !== $data['player']->username)
							{
								return false;								
							}
						}
	...
	private function adjacentCheck($x, $y, $z)
	{
		$item = $this->api->block->getBlock($x + 1, $y, $z);
		if($item->getID() === CHEST) return $item;
		$item = $this->api->block->getBlock($x - 1, $y, $z);
		if($item->getID() === CHEST) return $item;
		$item = $this->api->block->getBlock($x , $y, $z + 1);
		if($item->getID() === CHEST) return $item;
		$item = $this->api->block->getBlock($x , $y, $z - 1);
		if($item->getID() === CHEST) return $item;
		return false;
	...
	{
		$tmp = strtolower($item);
		if(isset($this->blocks[$tmp]) === true) return $item;
		if(isset($this->items[$tmp]) === true) return $item;
		if(($id = array_search($tmp, $this->blocks)) !== false) return $id;
		if(($id = array_search($tmp, $this->items)) !== false) return $id;
		return false;
	}

/* 	private function getNextChest($x, $y, $z)
	{
		$tile = $this->api->tileentity->get($x + 1, $y, $z);
		if($tile[0]->class === TILE_CHEST) return $tile[0];
		$tile = $this->api->tileentity->get($x - 1, $y, $z);
		if($tile[0]->class === TILE_CHEST) return $tile[0];
		$tile = $this->api->tileentity->get($x, $y, $z + 1);
		if($tile[0]->class === TILE_CHEST) return $tile[0];
		$tile = $this->api->tileentity->get($x, $y, $z - 1);
		if($tile[0]->class === TILE_CHEST) return $tile[0];
		return false;
	...
	private function startTransaction($data, $shopInfo, $c)
	{
		...
		$buyerMoney = $this->api->handle("money.player.get", $d);
		...
		$cmd = "give";
		$params = array($data['player']->username, $shopInfo['productID'], $shopInfo['saleNum']);
		$issuer = $this->api->player->get($data['player']->username);
		$alias = false;
